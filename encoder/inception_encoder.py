...
def conv_block(img_input,
    ...
    result = Conv(filters=nfilters, kernel_size=1, padding='same',
                   activation='relu', data_format=IMAGE_ORDERING, kernel_initializer=kernel_initializer, use_bias=False)(img_input)
    if instance_normalization is True:
        result = InstanceNormalization()(result)

    for i in range(num_convs - 1):
        result = Conv(filters=nfilters, kernel_size=1, padding='same',
                   activation='relu', data_format=IMAGE_ORDERING, kernel_initializer=kernel_initializer, use_bias=False)(result)
        if instance_normalization is True:
            result = InstanceNormalization()(result)
...
def basic_rdim_inception(img_input,
    ...
    tower_2 = Conv(filters=nfilters, kernel_size=3, padding='same',
                   activation='relu', data_format=IMAGE_ORDERING, kernel_initializer=kernel_initializer, use_bias=False)(tower_1)
    ...
    if not only_3x3_filters:
        ...
        tower_3 = Conv(filters=nfilters, kernel_size=3, padding='same',
                       activation='relu', data_format=IMAGE_ORDERING, kernel_initializer=kernel_initializer, use_bias=False)(tower_2)
    ...
    tower_4 = Conv(filters=nfilters, kernel_size=1, padding='same',
                   activation='relu', data_format=IMAGE_ORDERING, kernel_initializer=kernel_initializer, use_bias=False)(tower_4)
...
def basic_naive_inception(img_input, nfilters=64, IMAGE_ORDERING='channels_first', only_3x3_filters=False, carry_input=False, kernel_initializer=None):
    ...
    tower_1 = Conv(filters=nfilters,
                   ...
                   padding='same',
                   activation='relu',
                   data_format=IMAGE_ORDERING, use_bias=False)(img_input)
    ...
    if not only_3x3_filters:
        ...
        tower_3 = Conv(filters=nfilters,
                       ...
                       padding='same',
                       activation='relu',
                       data_format=IMAGE_ORDERING, use_bias=False)(tower_2)
...
#        print('superconv_output_shape:', _conc.output_shape)
    ...
    for i in range(skip_connections_treatment_number):
        ...
        if skip_connections_method == 'conv' or skip_connections_method == 'inception':
            ...
            layer_1 = cbam_block(layer_1, only_3x3_filters=only_3x3_filters)
            layer_2 = cbam_block(layer_2, only_3x3_filters=only_3x3_filters)
            layer_3 = cbam_block(layer_3, only_3x3_filters=only_3x3_filters)
            layer_4 = cbam_block(layer_4, only_3x3_filters=only_3x3_filters)
            layer_5 = cbam_block(layer_5, only_3x3_filters=only_3x3_filters)
