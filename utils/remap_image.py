def remap_image (input_image, output_image, order=3, contrast_stretching=1, scaling= 'WM'): #defaut order set to cubic function
    ...
    print('max:', max, 'reversalnum:', reversalnum)
    if max == 0:
        max = 1
...
if __name__ == '__main__':
    input_dir = 'D:\\university\\phd\\phd\\datasets\\sevilla24\\crop_48x64x64\\augmented\\'
    import os
    for item in os.listdir(input_dir):
        if 'seg' not in item and 'remap' not in item and 'augmented' not in item:
            remap_image(f'{input_dir}{item}', f'{input_dir}{item.replace(".nii", "-remap.nii")}')